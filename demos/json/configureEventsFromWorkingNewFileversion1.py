...
def enforce_dog_rules(events_by_dog, dog_rules):
	for dog in events_by_dog.keys():
		if dog_rules[dog][1] == noR:
			...
			parsed_statement = make_times_real(dog_rules[dog][1])
		current_start_time = ""
		for start_time in events_by_dog[dog]:
			start_time = make_times_real(start_time)
			if relativedelta(current_start_time,start_time) < 45:
				print "This dog has turns too close together"
			else:
				length_of_session = dog_rules[dog][0]
				if start_time+relativedelta(minutes=+length_of_session) not in events_by_dog["Break"]:
					print "This dog should have one longer turn, not two short ones"
